...
class FavoriteModel extends Model {
    ...
    public function getFavoriteListforWeibo($weibo_id, $page = 1, $count = 20) {
        
        $map['weibo_id'] = $weibo_id;
        $list = M('weibo_favorite')->field("uid")->where($map)->findPage($page);
        
        if(is_array($list)){
            foreach($list['data'] as $key => $val){
                $list['data'][$key]['uname'] = getUserName($val['uid']);
                $list['data'][$key]['face'] = getUserFace($val['uid']);
            }
        }
    ...
    public function getFavoriteCountforWeibo($weibo_id) {
        $map['weibo_id'] = $weibo_id;
        //var_dump($weibo_id);
        $list = M('weibo_favorite')->where("weibo_id=".$weibo_id)->count();
        return $list;
    }

    //收藏微博
    function favWeibo($weibo_id, $uid) {
        $data['uid'] = $uid;
        ...
        if ($res) {
            $this->_addFavoriteCache($uid, $weibo_id);
        }
        return $res;
    }
    ...
    function isFavoritedByDB($weibo_id, $uid, $static = false) {
        $map['weibo_id'] = $weibo_id;
        $map['uid'] = $uid;
    ...
    private function _getListData($uid) {
        ...
        self::$cacheHash[$uid]['data'] = $this->where("uid=$uid")->findAll();
        return self::$cacheHash[$uid]['data'];
    }
