...
weibo.prototype = {
    ...
    init:function(option){
        ...
        weibo.checkInputLength('#content_publish', _LENGTH_);
        shortcut('ctrl+return',	function(){
            weibo.do_publish();
            clearInterval(Interval);
        },{
            'target':'miniblog_publish'
        });
    },
    //发布前的检测
    ...
    share:function(id, upcontent){
        ...
        ui.box.load( U("group/WeiboOperate/shareWeibo",["id="+id,"gid="+weibo.gid,"upcontent="+upcontent] ),{
            title:'分享到我的微博',
            closeable:true
        });
    },
    ...
    transpond:function(id,upcontent){
        upcontent = ( upcontent == undefined ) ? 1 : 0;
        if(weibo.gid){
            ui.box.load( U("group/WeiboOperate/transpond",["id="+id,"gid="+weibo.gid,"upcontent="+upcontent] ),{
                title:'转发到群聊',
                closeable:true
            });
        }else{
            ui.box.load( U("weibo/operate/transpond",["id="+id,"upcontent="+upcontent] ),{
                title:'转发',
                closeable:true
            });
        }
    ...
    followTopic:function(name){
        $.post(U('weibo/operate/followtopic'),{
            name:name
        },function(txt){
            txt = eval( '(' + txt + ')' );
            if(txt.code==12){
    ...
    unfollowTopic:function(id,name){
        $.post(U('weibo/operate/unfollowtopic'),{
            topicId:id
        },function(txt){
            if(txt=='01'){
    ...
    quickpublish:function(text){
        $.post(U('weibo/operate/quickpublish'),{
            text:text
        },function(txt){
            ui.box.show(txt,{
                title:'说几句',
